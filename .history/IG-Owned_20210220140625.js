...
(function () {
  function checkIgOwned () {
    ...
    for (const el of $('a[href*=".indiegala.com/"]:not(".ig-checked")')) {
      ...
      if (/^https?:\/\/[\w\d]+?\.indiegala\.com\/.+$/.test(href) && allGames.includes(new URL(href).pathname.replace(/\//g, '').toLowerCase())) {
    ...
    return TM_request({
      ...
      .then(response => {
        ...
        if (response.status === 200) {
          ...
          const games = [...response.responseText.matchAll(/<a class="library-showcase-title" href="https:\/\/.*?\.indiegala\.com\/(.*?)" target="_blank">/g)].map(e => e[1].toLowerCase()).filter(e => e)
          return [pages, games]
        } else {
