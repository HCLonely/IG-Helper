    ...
    TM_request({
      ...
      .then(response => {
        if (response.status === 200) {
          console.log($(response.responseText).find('a.profile-private-page-library-pagination-item:has(.fa-angle-double-right)'))
          return
          if (page === 1) {
            const pages = $(response.responseText).find('a.profile-private-page-library-pagination-item:has(.fa-angle-double-right)')
          }
          const games = [...response.responseText.matchAll(/<a class="library-showcase-title" href="https:\/\/.*?\.indiegala\.com\/(.*?)" target="_blank">/g)].map(e => {
            return e[1]
          })
        }
      })
